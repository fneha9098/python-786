...
Sleek Two-Color Design (Dark Blue & Silver) with Enhanced Features
"""
...
class DatabaseManager:
    ...
    def create_tables(self):
        ...
        # Products table (base definition)
        cursor.execute('''
        ...
        # Add missing columns (no UNIQUE constraint to avoid errors)
        columns_to_add = {
            'min_stock': 'INTEGER DEFAULT 5',
            'location': 'TEXT',
            'supplier': 'TEXT',
        ...
        # Now create UNIQUE index on barcode if it doesn't exist and no duplicates
        try:
            cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_barcode ON products(barcode) WHERE barcode IS NOT NULL")
        except:
            pass
    ...
    def create_transaction(self, items: List[Dict], subtotal: float, tax: float, 
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO transactions (transaction_id, subtotal, tax_amount, discount_type,
                                        discount_amount, total_amount, payment_method, 
                                        cash_received, change_amount, customer_name, cashier_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (transaction_id, subtotal, tax, discount_type, discount_amount, 
                  total, payment_method, cash_received, change_amount, customer_name, cashier))
...
class GroceryStoreApp:
    ...
    def setup_pos_tab(self):
        """Professional POS Tab with two colors"""
        # Left panel - Products
        ...
        right_panel.pack(side="right", fill="both", padx=(5, 0), ipady=10)
        
        cart_header = ctk.CTkFrame(right_panel, fg_color="transparent")
        ...
        # Barcode input
        barcode_frame = ctk.CTkFrame(right_panel, fg_color="transparent")
        ...
        # Cart items
        self.cart_frame = ctk.CTkScrollableFrame(right_panel, fg_color=COLOR_PRIMARY, height=350)
    ...
    def setup_expenses_tab(self):
        ...
        self.exp_desc_var = ctk.CTkEntry(form_frame, width=200, fg_color=COLOR_SECONDARY)
        self.exp_desc_var.grid(row=0, column=3, padx=5, pady=5)
        
        ctk.CTkLabel(form_frame, text="Amount ($):").grid(row=0, column=4, padx=5, pady=5)
        self.exp_amount_var = ctk.CTkEntry(form_frame, width=100, fg_color=COLOR_SECONDARY)
        self.exp_amount_var.grid(row=0, column=5, padx=5, pady=5)
    ...
    def checkout(self):
        ...
        if self.payment_method_var.get() == "cash":
            try:
                ...
                messagebox.showerror("Error", "Please enter valid cash amount!")
                return
        ...
        if messagebox.askyesno("Confirm Checkout", 
            ...
            transaction_id = self.db.create_transaction(
                ...
                self.payment_method_var.get(), cash_received, self.customer_name_var.get(),
                self.current_cashier
            )
            
            if transaction_id:
                self.generate_receipt(transaction_id)
                messagebox.showinfo("Success", f"Transaction Complete!\nID: {transaction_id}\nChange: ${change:.2f}" if change else f"Transaction Complete!\nID: {transaction_id}")
                self.clear_cart()
                self.refresh_inventory()
                self.update_analytics()
                self.filter_products()
    ...
    def refresh_inventory(self):
        ...
        # Clear rows (keep header row at index 0)
        ...
        for i, product in enumerate(products, start=1):
            ...
            # Handle possible None for location
    ...
    def add_expense(self):
        try:
            ...
            amount = float(self.exp_amount_var.get())
    ...
    def load_expenses(self):
        ...
        # This is a simplified version - you can add a proper expenses table in the database
        ctk.CTkLabel(self.expenses_list_frame, text="Expense tracking feature - Add expenses table to database for full history",
                    text_color=COLOR_ACCENT).pack(pady=20)
